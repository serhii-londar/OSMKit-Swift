...
class OSMAPITest: XCTestCase {
    ...
    func testGetMap() {
        ...
        let expectation = self.expectation(description: "getMap")
        self.client.downloadBoundingBox(box) { (data, error) -> Void in
    ...
    func testGetNotes() {
        let box = BoundingBox(left: -122.27195, right: -122.26917, bottom: 37.87107, top: 37.87251)
        let expectation = self.expectation(description: "getNotes")
        self.client.downloadNotesBoundingBox(box) { (data, error) -> Void in
            XCTAssertNotNil(data)
            XCTAssertNil(error)
            expectation.fulfill()
        }
